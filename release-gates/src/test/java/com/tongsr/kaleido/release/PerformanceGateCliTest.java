...
public final class PerformanceGateCliTest {
    @Test
    public void acceptsMeasurementsAtAllBudgets() throws Exception {
        var values = passing();
        var verdict = PerformanceGateCli.evaluate(values);
        assertEquals(java.util.List.of(), verdict.failures());
        assertTrue(verdict.record().endsWith("verdict=PASS\n"));
    }

    @Test
    public void rejectsThresholdFailure() throws Exception {
        var values = passing();
        values.setProperty("plugin.jarBytes", Long.toString(10L * 1024 * 1024 + 1));
        assertEquals(1, PerformanceGateCli.evaluate(values).failures().size());
    }

    private static Properties passing() {
        ...
        for (var prefix : new String[] {"sana.safe", "sana.full", "sana.compose512",
                "sample.safe", "sample.full"}) {
